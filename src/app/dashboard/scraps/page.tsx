    ...
    <>
      <div className="flex flex-wrap justify-between items-end gap-4 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            본부가 스크랩한 기사 {articles.length + interScraps.length}건 (국내 {articles.length} · 해외 {interScraps.length}). 별표를 다시 누르면 해제됩니다.
          </p>
        </div>
      ...
      <div className="mb-6 rounded-xl border border-spark-light-purple bg-spark-light-purple/30 p-4 text-sm text-gray-700 leading-relaxed">
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li><b>해외(Inter) 스크랩</b>: 오른쪽 목록은 해외 트렌드 탭에서 스크랩한 기사입니다. 아직 다이제스트 본문에는 자동 포함되지 않고, 참고용으로 모아둡니다.</li>
        </ul>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
        ...
        <section className="bg-white rounded-xl border border-gray-200">
          <header className="flex items-center gap-2 border-b border-gray-100 px-5 py-3.5">
            ...
            <span className="text-sm font-bold text-spark-ink">국내 생태계</span>
            <span className="ml-auto text-xs tabular-nums text-gray-400">{articles.length}건</span>
          </header>
          <div className="overflow-x-auto p-4">
            <ArticlesTable
              ...
              emptyText="아직 스크랩한 국내 기사가 없습니다. 대시보드 기사 목록에서 ☆를 눌러 스크랩하세요."
            />
          </div>
        </section>
        ...
        <section className="bg-white rounded-xl border border-gray-200">
          <header className="flex items-center gap-2 border-b border-gray-100 px-5 py-3.5">
            ...
            <span className="text-sm font-bold text-spark-ink">해외 트렌드</span>
            <span className="ml-auto text-xs tabular-nums text-gray-400">{interScraps.length}건</span>
          </header>
          <div className="p-4">
            {interScraps.length === 0 ? (
              <p className="py-8 text-center text-sm text-gray-400">
                아직 스크랩한 해외 기사가 없습니다. 대시보드 <b>🔭 Inter</b> 탭의 분야를 펼쳐 ☆를 눌러 스크랩하세요.
              </p>
            ) : (
                  ...
                  <li key={v.id} className="flex items-start gap-2.5 py-2.5">
                    <div className="min-w-0 flex-1">
                      <div className="mb-1 flex flex-wrap items-center gap-1.5">
                        {v.domain && (
                          <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${v.domain === 'bio' ? 'bg-cyan-50 text-cyan-700' : 'bg-violet-50 text-violet-700'}`}>
                            {DOMAIN_LABEL[v.domain] ?? v.domain}
                          </span>
                        )}
                        ...
                        {v.matches.length > 0 && (
                          <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700">
                            ...
                            {v.matches.length > 1 ? ` 외 ${v.matches.length - 1}` : ''}
                          </span>
                        )}
                      </div>
                      <a href={v.news.url} target="_blank" rel="noopener noreferrer" className="block text-[13px] font-semibold leading-snug text-spark-ink hover:text-spark-purple">
                        {v.titleKo || v.news.title}
                      </a>
                      {v.titleKo && v.titleKo !== v.news.title && (
                      ...
                      <div className="mt-0.5 text-[11px] text-gray-400">
                        ...
                        {v.scrappedAt && ` · 스크랩 ${fmtDate(v.scrappedAt)}`}
                      </div>
                    </div>
