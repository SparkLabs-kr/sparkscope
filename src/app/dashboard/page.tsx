...
async function loadDashboardData(from: string, to: string, company?: string) {
  ...
  const [
    ...
    prisma.article.findMany({ where: sparklabsWhere, orderBy: { pubDate: 'desc' }, select: { id: true, title: true, link: true, source: true, pubDate: true, tone: true }, take: 300 }),
  ...
  if (company) {
    ...
    companyArticles = rows.filter(a => keys.some(k => matchesAsToken(a.title, k)));
  }
