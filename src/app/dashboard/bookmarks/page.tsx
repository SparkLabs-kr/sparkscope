...
export default async function BookmarksPage() {
  ...
  if (getLocale() === 'en') await ensureArticleEnDeep([ordered]);

  return (
    <>
      <div className="flex flex-wrap justify-between items-end gap-4 mb-4">
        <div>
          <h1 className="text-3xl font-bold">🔖 {t('내 북마크')}</h1>
          <p className="text-sm text-gray-500 mt-1">{t('내가 저장한 기사 {n}건. 나만 보이고, 다른 사람 목록에는 안 나타납니다.', { n: ordered.length })}</p>
        </div>
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-200">
        <ArticlesTable
          ...
          canBookmark
          emptyText={t('아직 북마크한 기사가 없습니다. 대시보드의 🔖 아이콘을 눌러 저장하세요.')}
        />
      </div>
    </>
  );
