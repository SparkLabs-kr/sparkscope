...
import { packForCategory } from './locale';
...
function computePriorityScore(article: RawArticle, importance: Importance, tone: Tone): number {
  ...
  // 메이저 매체 가중치 — 로케일 팩이 판정한다. 한국은 고정 목록, 대만은 Tier 1·2.
  // 예전엔 여기 한국 매체 목록이 하드코딩돼 있어 聯合報·工商時報가 실려도 0점이었다.
  const mediaBonus = packForCategory(article.category).media.isMajor(article.source) ? 15 : 0;
  // 신선도
