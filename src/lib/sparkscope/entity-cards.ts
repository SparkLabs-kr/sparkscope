...
export async function buildEntityCards(
  ...
  return foldSubEntities(cards)
    ...
    // 카드가 되려면 무언가를 더 보여줘야 한다 — 커뮤니티 반응이 있거나, 기사가 둘 이상.
    //
    // 이 패널은 "여러 매체가 함께 말한 이름과, 커뮤니티에서 실제로 다뤄진 정도"다.
    // 그런데 매체 수만으로 줄을 세우니 반응도 없고 기사도 하나뿐인 이름이 올라왔다 —
    // 2026-09-11 지적: '포지트론' 카드가 매체 2·기사 1·반응 0이었다. 그 기사 하나는
    // 이미 왼쪽 목록에 있으므로, 카드는 같은 것을 한 번 더 보여준 것뿐이다.
    //
    // 반응이 없어도 기사가 둘 이상이면 남긴다 — "이 이름으로 여러 사건이 돌고 있다"는
    // 정보다.
    //
    // 2026-09-11에는 이 문턱을 AI에만 걸었다. 바이오에 같은 규칙을 걸면 카드가
    // 6개에서 2개로 줄고 남은 둘이 모두 기사 0건이어서, "여러 매체가 이 이름을
    // 말했다"가 바이오의 유일한 신호라고 판단했다.
    //
    // 그 판단의 전제가 틀려 있었다. 당시 바이오 커뮤니티에서 점수가 붙는 소스는
    // Hacker News 하나뿐인데 그게 2026-09-08부터 멈춰 있었고(도메인을 구분하지 않는
    // 수집 주기 + AI 글이 바이오 칸을 덮는 문제, social-collect.ts 참고), 게다가
    // 들어오던 것마저 AI 글이었다. 반응이 붙을 수 없는 상태에서 "반응이 없다"를
    // 근거로 문턱을 뺀 셈이다.
    //
    // 그걸 고친 뒤 재보니 바이오 카드에도 반응이 붙는다(2026-09-18: 모더나 840점,
    // Thermo Fisher 447점). 반면 문턱이 없는 탓에 매체 수만으로 올라온 카드가
    // 11개 중 8개였고 — 전부 반응 0·기사 1 — 그 기사는 이미 왼쪽 목록에 있으므로
    // 카드가 같은 것을 한 번 더 보여준 것뿐이었다(사용자 지적: "시그널이 없는데?").
    //
    // 그래서 두 도메인에 같은 규칙을 적용한다. 바이오 카드 수는 줄지만, 남는 카드는
    // 전부 왼쪽 목록에 없는 것을 더한다.
    .filter(c => c.community.length > 0 || c.articles.length >= 2)
