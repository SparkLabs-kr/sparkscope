...
export async function buildEntityCards(
  ...
  return foldSubEntities(cards)
    ...
    // 정보다.
    //
    // 이 문턱은 AI에만 적용한다. 바이오 쪽 커뮤니티는 bioRxiv·PubMed·임상등록이고
    // 논문 제목에 회사 이름이 나오는 일이 드물어서, 같은 규칙을 걸면 기사 쪽 카드가
    // 전멸한다(2026-09-11 실측: 바이오 카드 6개 → 2개, 남은 둘은 모두 기사 0건).
    // 바이오에서는 "여러 매체가 이 이름을 말했다"가 사실상 유일한 신호다.
    .filter(c => domain === 'bio' || c.community.length > 0 || c.articles.length >= 2)
