...
- 퍼센트(%) 증감률은 쓰지 마라. 늘었다/줄었다는 "이번 기간 N건, 직전 기간 M건"처럼 건수로 말한다.
...
- 이번 기간 건수와 직전 기간 건수를 나란히 말하고, 그 차이가 큰 편인지 판단한다(퍼센트 금지).
...
- 직전 기간 건수보다 늘었으면 급증 신호로 볼 수 있는지 판단한다.
...
1) 핵심 요약 — 기간과 건수(이번/직전)를 담은 2~3문장
...
function buildPrompt(question: string, r: ChatQueryResult, intent: ChatIntent) {
  ...
  if (r.prevTotal !== null) {
    lines.push(`[직전 같은 기간] ${r.prevTotal}건`);
