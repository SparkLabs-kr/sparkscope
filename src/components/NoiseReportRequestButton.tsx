...
export function NoiseReportRequestButton({ id }: { id: string }) {
  ...
  async function submit() {
    ...
    if (res.ok) {
      ...
      setError(data.error ?? t('신고 접수에 실패했습니다.'));
    }
  }
  ...
  if (sent) {
    return (
      <span
        title={t('신고가 접수됐습니다. 관리자 검토 후 처리됩니다.')}
  ...
  return (
    <span className="relative inline-flex">
      ...
      {open && (
        ...
        >
          <div className="mb-1 text-[11px] font-semibold text-gray-700">{t('노이즈 신고 사유')}</div>
          <textarea
            value={reason}
            onChange={e => setReason(e.target.value)}
            rows={2}
            placeholder={t('예: 회사와 무관한 동명이인 기사입니다')}
