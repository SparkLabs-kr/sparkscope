...
export function InterBriefingModal({ payload, onClose }: { payload: BriefingPayload; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/inter/briefing', {
      ...
      .then(async res => {
        ...
        if (cancelled) return;
        if (!res.ok || !json?.html) {
          setError(json?.error ?? t('브리핑을 만들지 못했습니다.'));
          return;
        }
        setHtml(json.html);
        setIsAi(!!json.isAi);
      ...
      .catch(() => { if (!cancelled) setError(t('브리핑을 만들지 못했습니다.')); });
  ...
  async function copy() {
    ...
    try {
      ...
      setNotice(t('복사했습니다 — 메일 본문에 붙여넣으세요.'));
    } catch {
      setNotice(t('복사에 실패했습니다.'));
    }
  }
  ...
  async function sendToMe() {
    ...
    try {
      ...
      const json = await res.json().catch(() => null);
      setNotice(res.ok && json?.sentTo ? t('{to} 로 보냈습니다.', { to: json.sentTo }) : (json?.error ?? t('발송에 실패했습니다.')));
    } catch {
      setNotice(t('발송에 실패했습니다.'));
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t('{company} 브리핑 미리보기', { company: payload.company })}
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="flex items-center gap-2 border-b border-spark-border px-4 py-3">
          <div className="min-w-0">
            <div className="text-[14px] font-bold text-spark-ink">{t('{company} 브리핑', { company: payload.company })}</div>
            <div className="truncate text-[11px] text-spark-muted">
              {payload.domainLabel} · {payload.sector.name} · {payload.periodLabel} · {t('기사 {n}건', { n: payload.articles.length })}
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto scroll-slim bg-spark-subtle/50 p-3">
          ...
          <div className="mb-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] leading-relaxed text-amber-900">
            <b>{t('보내는 방법')}</b> · {t('아래')} <b>{t('복사')}</b>{t('를 누르면 서식이 살아 있는 상태로 클립보드에 담깁니다. Gmail·Outlook 새 메일 본문에 그대로 붙여넣어 포트폴리오사에 보내세요.')}{' '}
            <b>{t('내 메일로 받기')}</b>{t('를 쓰면 본인 메일함으로 받아 전달(Forward)할 수도 있습니다.')}
            <span className="text-amber-700"> {t('(이 안내문은 복사·발송되는 내용에는 포함되지 않습니다.)')}</span>
          </div>
          {error ? (
            ...
            <div className="py-12 text-center text-[13px] text-spark-muted">
              {t('브리핑을 만드는 중입니다… (AI 요약에 10초 안팎 걸립니다)')}
            </div>
          ) : (
        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-spark-border px-4 py-3">
          <span className="text-[11px] text-spark-muted">
            {html
              ? isAi
                ? `🤖 ${t('AI 요약 포함')}${cached ? ` · ${t('이전에 만든 요약 재사용')}` : ''}`
                : t('⚙️ 기본 요약(AI 호출 실패 — 지표 기반 문구)')
              : ''}
          </span>
