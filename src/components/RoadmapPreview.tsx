...
function PreviewTag({ t }: { t: Translate }) {
  return (
  ...
  return (
    <div className="mt-10">
      ...
      <div className="flex items-center gap-3 mb-1">
        <h2 className="text-xl font-bold text-spark-purple">🚀 {t('고도화 미리보기')}</h2>
        <PreviewTag t={t} />
      </div>
      <p className="text-sm text-gray-500 mb-5">
        {t('아래는 다음 단계에서 추가될 기능의')} <b>{t('예시 화면')}</b>{t('입니다. 빠른 시일 내 개발하겠습니다.')}
      </p>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-5">
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-200">
          <div className="flex justify-between items-center mb-4">
            <div className="font-bold">🌐 {t('데이터 소스 커버리지')}</div>
            <PreviewTag t={t} />
          </div>
          <div className="space-y-2.5">
            <SourceRow name={t('구글 뉴스')} status={t('연동 완료')} done />
            <SourceRow name={t('네이버 뉴스')} status={t('연동 완료')} done />
            <SourceRow name={t('링크드인')} status={t('연동 예정')} />
            <SourceRow name={t('인스타그램')} status={t('연동 예정')} />
          </div>
          <p className="text-xs text-gray-400 mt-4">{t('출처를 넓혀 기사 누락을 최소화합니다')}</p>
        </div>
      </div>
