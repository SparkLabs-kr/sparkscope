...
function ToneLine({ tones, t }: { tones?: SourceTones; t: Translate }) {
  if (!tones) return null;
  const parts: { text: string; cls: string }[] = [];
  if (tones.POSITIVE) parts.push({ text: t('긍정 {n}', { n: tones.POSITIVE }), cls: 'text-emerald-600' });
  if (tones.NEUTRAL) parts.push({ text: t('중립 {n}', { n: tones.NEUTRAL }), cls: 'text-gray-500' });
  if (tones.NEGATIVE) parts.push({ text: t('부정 {n}', { n: tones.NEGATIVE }), cls: 'text-spark-muted' });
  if (parts.length === 0) return null;
  if (parts.length === 1) {
    return <span className="shrink-0 text-[11px] whitespace-nowrap"><span className="text-spark-muted">{t('전부')} </span><span className={parts[0].cls}>{parts[0].text}</span></span>;
  }
  return (
...
export function MediaPanel({ data, defaultCount = 5 }: { data: SourceRow[]; defaultCount?: number }) {
  ...
  if (data.length === 0) {
    return <p className="text-sm text-gray-400 py-8 text-center">{t('선택 기간 내 매체 노출 데이터가 없습니다.')}</p>;
  }
  ...
  return (
    <div>
        ...
        {shown.map(d => {
          ...
          return (
            <div key={d.source} className="flex items-center gap-2">
              <span className={`w-24 shrink-0 text-xs text-right truncate ${isTopPositive ? 'font-bold text-spark-ink' : 'text-spark-ink-soft'}`} title={t(d.source)}>{t(d.source)}</span>
      ...
      {data.length > defaultCount && (
        ...
        >
          {expanded ? t('접기') : t('더보기 (전체 {n}개 매체)', { n: data.length })}
        </button>
      )}
    </div>
  );
